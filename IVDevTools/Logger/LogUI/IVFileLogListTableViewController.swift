...
class IVFileLogListTableViewController: UIViewController {
    ...
    @IBAction func trashClicked(_ sender: UIButton) {
        if let selectedRows = tableView.indexPathsForSelectedRows?.sorted(by: { $0 > $1}) {
            ...
            selectedRows.forEach { (idxPath) in
                IVFileLogger.shared.deleteLogFile(dataSource[idxPath.row])
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let vc = segue.destination as? IVFileLogTextViewController, let indexPath = tableView.indexPathForSelectedRow {
    ...
    deinit {
        print(#function, "IVFileLogListTableViewController")
    }
}

extension IVFileLogListTableViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if dataSource[indexPath.row] == IVFileLogger.shared.currLogFileURL {
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let delete = UITableViewRowAction(style: .destructive, title: "删除", handler: { _,_ in
            ...
            alert.addAction(UIAlertAction(title: "删除", style: .destructive, handler: { _ in
                IVFileLogger.shared.deleteLogFile(self.dataSource[indexPath.row])
                self.dataSource = IVFileLogger.shared.getAllFileURLs(pathExtension: self.dataSource[0].pathExtension)
